...
%         disp(toc_Notify)
        ...
        if length(deviceData) == gTotalChannels * gSamplesPerChannelPerBlock + 1 % Just to check that the correct amount of data points has been received.
            ...
            for m = 1:size(gChannelsRequired,2)
                ...
                end_sample = gChannelsRequired(1,m) * gSamplesPerChannelPerBlock + 1 - (gDecFactor - 1);
                gAccumData(:,m) = deviceData(start_sample:gDecFactor:end_sample)';
            end
        else
...
%             device.Configuration = strConfiguration;
            ...
            gAccumData = zeros(gSamplesPerChannelPerBlock/gDecFactor,size(gChannelsRequired,2));
